...
class DigitPrompter(prompter.Prompter):

    digit_prompt = """
<Instruction> Calculate the given sequence. Output only number, no additional text or list. </Instruction>
...
"""
    ...
    tot_improve_prompt = """
<Instruction> There are some errors in the following calculation sequence. Fine the errors in it and correct it. Only output the answer(one number),  no additional text or list.</Instruction>
...
Only output the final number without any additional text ,thoughts or list:</Instruction>
...
"""
    ...
    def generate_prompt(
        ...
        if method.startswith("io"):
            return self.digit_prompt.format(input=input)
            ...
            if kwargs["phase"] == 1:
                return self.digit_prompt.format(input=current)
...
class DigitParser(parser.Parser):
    """
    ...
    def parse_aggregation_answer(
        ...
        for text in texts:
            ...
            high = int(states[0]["current"])  # 12483
            low  = int(states[1]["current"])  # 12590
...
def run(
    ...
    orig_budget = budget
    data_path = os.path.join(os.path.dirname(__file__), "digit_8.csv")
    data = []
    ...
    for data in selected_data:
        ...
        for method in methods:
            ...
            results['output'].append(final_ans[0] if type(final_ans) == type([]) else final_ans)
